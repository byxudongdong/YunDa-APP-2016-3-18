...
public class ReturnPartDatauploadActivity extends Activity implements OnItemClickListener{
	Button search;
	Spinner spinner;
	ListView listView;
	String str;
	...
	ImpDataUpload db;
	List<PartData> list;// 实体类的各个实体
	BaseAdapter adapter;
	public DatePickerDialog d;
	String times="";
	private PartData mPartData;
	private static String URL_INTENT = "";
	private ImpReturnPartData mImpDataUpload;
	private MyApplication application;
	private List<PartData> vList=new ArrayList<PartData>();
	...
	private void XmlPersor(String result) {
	 ...
	 if (st.equals("ok")) {
		 ...
		 Iterator<PartData> it=vList.iterator();
		 int resultttt =Integer.parseInt(res);
		 while (it.hasNext()&&resultttt>0) {
			 PartData mPartData=it.next();
			 mImpDataUpload. updateInformation(mPartData.getNumber(),1);
			 	it.remove();	
			 	resultttt--;	 	 
		 }
		 Log.i("mImpDataUpload", mPartData.getNumber());		 
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		switch (keyCode) {
		 case 131://F1再次上传	:
		Iterator<PartData>	iterator=list.iterator();
		while (iterator.hasNext()) {
			PartData mPartData=iterator.next();
			if (Integer.parseInt(mPartData.getFlag())==0) {
				vList.add(mPartData);	
			}		
		}							
	requestIntent(new PartData(vList, application.getConpanyCode(),application.getUserID(), application.getProfessionID(), getDate(), application.getPassWord()).toString());			
			break;
		 case 132://F2锁屏:
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		spinner.setAdapter(new BaseAdapter() {
			...
			public View getView(int position, View convertView, ViewGroup parent) {
				...
				convertView = LayoutInflater.from(ReturnPartDatauploadActivity.this).inflate(R.layout.date, null);
		...
		search.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				case 3:
                    ...
                    c.setTime(new Date());
					if (d == null) {
						d = new DatePickerDialog(ReturnPartDatauploadActivity.this, new OnDateSetListener() {
							@Override
		...
		listView.setAdapter(adapter = new BaseAdapter() {
			...
			public View getView(int position, View convertView, ViewGroup parent) {
				...
				if (convertView == null) {
					...
					convertView = LayoutInflater.from(ReturnPartDatauploadActivity.this).inflate(R.layout.dateitem, null);
				 ...
				 mPartData = list.get(position);
				holder.yiguo.setText(Integer.parseInt(mPartData.getFlag()) == 0 ? "未" : "已");// 判断flag是上传还是没有上传
				holder.yundanhao.setText(mPartData.getNumber());
				return convertView;
			}
	...
	//	back = (Button) findViewById(R.id.back_button);
		...
		mImpDataUpload=new ImpReturnPartData(this);
		list=mImpDataUpload.QueryProfession(getDates());
	...
	public void AlertDialog(final int position) {
		LayoutInflater inflater=LayoutInflater.from(ReturnPartDatauploadActivity.this);
		View view =	inflater.inflate(R.layout.lookreturnxml, null);
		...
		textView1.setText(list.get(position).getTypework());
		textView2.setText(list.get(position).getTypeworkid());
		textView3.setText(list.get(position).getNumber());
		textView4.setText(list.get(position).getTime());
		textView6.setText(list.get(position).getCurrentsite());
		textView5.setText(Integer.parseInt(list.get(position).getFlag())==0 ?"未上传":"已上传");
		textView7.setText(list.get(position).getUserid()+":"+list.get(position).getName());
		Builder builder = new AlertDialog.Builder(ReturnPartDatauploadActivity.this);
